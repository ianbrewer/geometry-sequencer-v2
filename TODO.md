...
- [ ] **Stage G — Cleanup: retire legacy types + `custom`** — `feat/asset-cleanup-stage-g`. Gated on the Supabase audit (see "Legacy shape-type code removal audit" below). Once audit is clean: delete the astrology/amino/iching_lines branches in `GeometryRenderer` + `PrimitiveRenderer`, remove `ASTRO_PATHS` / `AMINO_PATHS` imports in the renderer (keep the files — `seedAssets.ts` still reads them; delete them only once Stage G is merged and no new users need seeding from source, or refactor seed generation to bake JSON), strip those options from the `ShapeType` union. Handle `custom` separately: on project load, upload any layer's `customPath`/`customPaths` as an SVG asset in a "Custom SVGs" folder, then rewrite the layer to `asset_single`; remove `handleFileUpload` from `Inspector.tsx` and `custom` from the type union.
...
- [ ] **Legacy shape-type code removal audit** — run [scripts/audit-legacy-shape-types.sql](./scripts/audit-legacy-shape-types.sql) against the shared Supabase project to confirm zero rows in `projects.layers` still reference `astrology` / `amino` / `iching_lines`. Stage F migrates those types to `asset_set` on load, so rows persist legacy types only until their next save. Once the query returns zero rows (or the remaining stragglers are acceptable to break), Stage G deletes the legacy code paths. `custom` is on a separate deprecation track — see Stage G notes.
