...
import { getUnitPolygon, getUnitCustomShape, getUnitStar } from '../utils/geometry';
import { drawIChingHexagram } from './shapes/IChingRenderer';
...
export const updatePrimitive = (g: Graphics, type: ShapeType, rx: number, ry: number, sides: number, density: number, layerConfig: LayerConfig, progress: number, currentStrokeWeight: number, currentShapeArc: number = 360, currentStarInnerRadius: number = 0.5, rotateX: number = 0, rotateY: number = 0, currentPerspective: number = 1200) => {
    ...
    const applyFill = () => {
        if (layerConfig.fillEnabled) g.fill(fillColor);
    };
    ...
    switch (type) {
        case 'polygon': {
            const points = (layerConfig.customPath) ? getUnitCustomShape(layerConfig.customPath) : getUnitPolygon(sides);
            ...
            // 2. Draw Stroke
            if (layerConfig.strokeEnabled && layerConfig.drawOutline !== false) {
                if (isDashed) {
            ...
            if (layerConfig.dotsEnabled) {
                ...
                points.forEach(p => {
                    ...
                    if (layerConfig.dotType === 'filled') g.fill(layerConfig.strokeColor || '#fff');
        ...
        case 'circle': {
            if (layerConfig.fillEnabled) {
                if (currentShapeArc >= 360) g.ellipse(0, 0, rx, ry);
                    ...
                    g.moveTo(0, 0);
                    ellipticalArc(g, 0, 0, rx, ry, 0, currentShapeArc * (Math.PI / 180));
                    g.closePath();
                }
                applyFill();
            }
            ...
            if (layerConfig.strokeEnabled) {
                ...
                if (isDashed) {
                    ...
                    if (currentShapeArc >= 360) g.ellipse(0, 0, rx, ry);
                    else ellipticalArc(g, 0, 0, rx, ry, 0, endAngle);
                }
                strokeCurrent();
            }
